...
public class GameHelper {
        ...
        for (int i = 0; i < state.getBoard().length; i++) {
            for (int j = 0; j < state.getBoard().length; j++) {
                ...
                if (state.getPawn(i, j).equalsPawn(State.Pawn.EMPTY.toString())
                        && !(campsList.stream().anyMatch(camp -> Arrays.equals(camp, pawn)))) {
                    buf = new int[2];
                    buf[0] = i;
                    buf[1] = j;
                    empty.add(buf);
    ...
    public static List<Action> getPawnMoves(State state, int[] pawn) {
        ...
        int row = pawn[0];
        int column = pawn[1];
        // if(state.getTurn().equals(State.Turn.BLACK)) System.out.println("row: "+row+"
        // column: "+column);
    ...
    public static float getKingOpenFiles(State state) {
        ...
        int i = 0;
        if (king_pos[0] == 1 || king_pos[0] == 2 || king_pos[0] == 6 || king_pos[0] == 7) {
            i = king_pos[1] - 1;
            while (i >= 0) {
        ...
        if (king_pos[1] == 1 || king_pos[1] == 2 || king_pos[1] == 6 || king_pos[1] == 7) {
            i = king_pos[0] - 1;
            while (i >= 0) {
